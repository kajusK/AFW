...
static const uint8_t scsi_inquiry_sn_data[] = { 0x00, 0x80, 0x00,
 ...
 */
static void scsi_set_status(msc_desc_t *ms, scsi_sense_key_t key, scsi_sense_asc_t asc,
    scsi_sense_ascq_t ascq)
{
    ms->sense.key = key;
    ms->sense.asc = asc;
    ms->sense.ascq = ascq;
...
{
    scsi_set_status(ms, SENSE_KEY_NO_SENSE, ASC_NO_ADDITIONAL_SENSE_INFORMATION, ASCQ_NA);
}
...
static void scsi_verify_rw_range(msc_desc_t *ms)
{
    if (ms->trans.lba_start + ms->trans.block_count - 1 > ms->block_count) {
        scsi_set_status(ms, SENSE_KEY_ILLEGAL_REQUEST, ASC_LBA_OUT_OF_RANGE, ASCQ_NA);
    } else {
...
static void scsi_write_10(msc_desc_t *ms)
{
    ...
    buf = ms->trans.cbw.CBWCB;
    ms->trans.lba_start = (buf[2] << 24) | (buf[3] << 16) | (buf[4] << 8) | buf[5];
...
static void scsi_request_sense(msc_desc_t *ms)
{
    ...
    memcpy(ms->trans.msd_buf, scsi_request_sense_data, sizeof(scsi_request_sense_data));
...
static void scsi_inquiry(msc_desc_t *ms)
{
    ...
    if (0 == evpd) {
        ...
        memcpy(ms->trans.msd_buf, scsi_inquiry_data, sizeof(scsi_inquiry_data));
        ...
        memcpy(ms->trans.msd_buf, scsi_inquiry_sn_data, sizeof(scsi_inquiry_sn_data));
...
static void scsi_command(msc_desc_t *ms)
{
    ...
    switch (ms->trans.cbw.CBWCB[0]) {
        ...
        case SCSI_PREVENT_ALLOW_MEDIUM_REMOVAL:
            ...
            break;
        default:
            scsi_set_status(ms, SENSE_KEY_ILLEGAL_REQUEST, ASC_INVALID_COMMAND_OPERATION_CODE,
                ASCQ_NA);
            break;
    }
}
...
static void msci_data_rx(usbd_device *usbd_dev, uint8_t ep)
{
    ...
    if (left > 0) {
        trans->cbw_cnt +=
            usbd_ep_read_packet(usbd_dev, ep, &((uint8_t *)&trans->cbw)[trans->cbw_cnt], left);
...
static void msci_data_tx(usbd_device *usbd_dev, uint8_t ep)
{
    ...
    if (left > 0) {
        trans->csw_sent +=
            usbd_ep_write_packet(usbd_dev, ep, &((uint8_t *)&trans->csw)[trans->csw_sent], left);
    } else if (sizeof(trans->csw) == trans->csw_sent) {
        scsi_finish_transaction(trans);
    }
}
 ...
 */
static enum usbd_request_return_codes msci_control_request(usbd_device *usbd_dev,
...
static void msci_set_config(usbd_device *usbd_dev, uint16_t wValue)
{
    ...
    usbd_ep_setup(usbd_dev, desc->ep_in, USB_ENDPOINT_ATTR_BULK, desc->ep_in_size, msci_data_tx);
    usbd_ep_setup(usbd_dev, desc->ep_out, USB_ENDPOINT_ATTR_BULK, desc->ep_out_size, msci_data_rx);
 ...
 */
void Msc_Init(usbd_device *usbd_dev, uint8_t ep_in, uint8_t ep_in_size, uint8_t ep_out,
    uint8_t ep_out_size, const char *vendor_id, const char *product_id,
    const char *product_revision_level, msc_read_block_t read_block, msc_write_block_t write_block,
    uint32_t block_count)
{
