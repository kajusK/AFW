...
static int32_t TCi_GetVoltage(const int32_t lookup[][2],
        uint16_t lookup_size, int32_t cold_temp_mc)
{
    uint16_t i;
    int32_t x1, x2, y1, y2;
...
static int32_t TCi_GetTemp(const int32_t lookup[][2], uint16_t lookup_size,
        int32_t voltage_uv, int32_t cold_temp_mc)
{
    uint16_t i;
    int32_t x1, x2, y1, y2;
...
int32_t TC_JConvertmC(uint32_t voltage_uv, int32_t cold_temp_mc)
{
    const int32_t lookup[][2] = {
            {-200, -7890}, {-150, -6500}, {-100, -4633}, {-50, -2431},
            {0, 0}, {50, 2585}, {100, 5269}, {150, 8010}, {200, 10779},
            {250, 13555}, {300, 16327}, {400, 21848}, {500, 27393},
            {600, 33102}, {700, 39132}, {800, 45494}
    };

    return TCi_GetTemp(lookup, sizeof(lookup)/sizeof(lookup[0]), voltage_uv,
            cold_temp_mc);
}
...
int32_t TC_KConvertmC(uint32_t voltage_uv, int32_t cold_temp_mc)
{
    ...
    const int32_t lookup[][2] = {
            {-200, -5891}, {-100, -3554}, {-50, -1889}, {0, 0},
            {50, 2023}, {100, 4096}, {150, 6138}, {200, 8138},
            {300, 12209}, {400, 16397}, {500, 20644}, {600, 24905},
            {700, 29129}, {800, 33275}, {900, 37326}, {1000, 41276},
            {1100, 45119}, {1200, 48838}, {1300, 52410}, {1370, 54819}
    };
    return TCi_GetTemp(lookup, sizeof(lookup)/sizeof(lookup[0]), voltage_uv,
            cold_temp_mc);
}
...
int32_t LMT87_ConvertmC(uint16_t voltage_mv)
{
    ...
    const int16_t lookup[][2] = {
            {-50, 3277}, {-40, 3160}, {-30, 3030}, {-20, 2899}, {-10, 2767},
            {0, 2633}, {10, 2500}, {20, 2365}, {30, 2231}, {40, 2095},
            {50, 1958}, {60, 1819}, {70, 1679}, {80, 1539}, {90, 1399},
            {100, 1257}, {110, 1115}, {120, 973}, {130, 829}, {140, 684},
            {150, 538}
    };
    uint16_t i;
    int16_t x1, x2, y1, y2;
