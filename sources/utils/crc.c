...
/**lookup table, generated using sw/tools/gen_crctable.py 0x31 8 */
static const uint8_t crc8i_lut_table[256] = { 0x00, 0x31, 0x62, 0x53, 0xc4, 0xf5, 0xa6, 0x97, 0xb9,
    ...
    0x0a, 0x59, 0x68, 0xff, 0xce, 0x9d, 0xac };

/** lookup table, generated using sw/tools/gen_crctable.py 0x1021 16 */
static const uint16_t crc16i_lut_table[256] = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5,
...
uint16_t CRC16_Add(uint8_t buf, uint16_t crc)
{
    ...
    index = buf ^ (crc >> 8);
    crc = crc16i_lut_table[index] ^ (crc << 8);
    return crc;
}
...
uint16_t CRC8_Add(uint8_t buf, uint8_t crc)
{
    ...
    index = (buf ^ crc) & 0xff;
    crc = crc8i_lut_table[index] ^ (crc << 8);
    return crc;
}

uint8_t CRC8(const uint8_t *buf, uint32_t len)
{
    uint16_t crc = CRC8_INITIAL_VALUE;
    uint32_t i;
